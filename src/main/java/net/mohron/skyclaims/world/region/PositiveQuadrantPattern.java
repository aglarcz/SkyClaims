...
public class PositiveQuadrantPattern implements IRegionPattern {
	...
	private static Logger logger = SkyClaims.getInstance().getLogger();
	...
	public Region nextRegion() throws InvalidRegionException {
		...
		logger.debug(String.format("Checking for next region out of %s points with %s spawn regions.", regions.size(), PositiveQuadrantPattern.spawnRegions));
