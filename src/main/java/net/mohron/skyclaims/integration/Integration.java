...
public class Integration {
	...
	public Integration() {
		...
		if (Sponge.getPluginManager().getPlugin("mcclans").isPresent()) {
			...
			SkyClaims.getInstance().getLogger().info("Found MCClans " + version);
			if (Integer.parseInt(version) >= PluginInfo.MCCLANS_API_VERSION) mcclans = new MCClans();
		}
	}
